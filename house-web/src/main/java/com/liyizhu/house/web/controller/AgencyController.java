...
import com.liyizhu.house.biz.service.HouseService;
import com.liyizhu.house.biz.service.MailService;
import com.liyizhu.house.biz.service.RecommendService;
...
public class AgencyController {
    ...
    public String agencyCreate(){
        ...
        if (user == null || !Objects.equal(user.getEmail(), "1846782779@qq.com")) {
            return "redirect:/accounts/signin?" + ResultMsg.successMsg("请先登录超级管理员账号").asUrlParams();
        }
    ...
    public String agentMsg(Long id,String msg,String name,String email, ModelMap modelMap){
        User user =  agencyService.getAgentDeail(id);
        mailService.sendEmail("咨询", "email:"+email+",msg:"+msg, user.getEmail());
    ...
    public String agencySubmit(Agency agency){
        User user =  UserContext.getUser();
        if (user == null || !Objects.equal(user.getEmail(), "1846782779@qq.com")) {//只有超级管理员可以添加
